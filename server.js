...
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || 'postgresql://postgres:plhZVkkOfWzNSdDbvxAcKSkdLdZBuZbI@postgres.railway.internal:5432/railway',
  ssl: false
});
...
app.get('/api/trips', async (req, res) => {
  try {
    const result = await pool.query("SELECT data FROM trips ORDER BY (data->>'date') ASC, (data->>'time') ASC");
    res.json(result.rows.map(r => r.data));
  } catch (err) { console.error(err.message); res.json([]); }
});
